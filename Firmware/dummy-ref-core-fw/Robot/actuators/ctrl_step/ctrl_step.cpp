...
CanTxStatus CtrlStepMotor::TrySetEnable(bool _enable,
...
CanTxStatus CtrlStepMotor::SendPositionSetPoint(float _val, bool request_ack,
                                                bool non_blocking,
                                                const CanTxMetadata* metadata)
{
    uint8_t mode = 0x05;
    ...
    return CanSendMessage(get_can_ctx(hcan), canBuf, &txHeader)
        ? CanTxStatus::Queued : CanTxStatus::Error;
}
...
CanTxStatus CtrlStepMotor::SendPositionWithVelocityLimit(float _pos, float _vel,
                                                          bool non_blocking,
                                                          const CanTxMetadata* metadata)
{
    uint8_t mode = 0x07;
...
CanTxStatus CtrlStepMotor::TrySetVelocityLimitRam(
...
CanTxStatus CtrlStepMotor::TryGetTemp(const CanTxMetadata* metadata)
{
    constexpr uint8_t mode = 0x25;
    CAN_TxHeaderTypeDef request_header = txHeader;
    request_header.StdId = nodeID << 7 | mode;
...
CanTxStatus CtrlStepMotor::TryGetTimingProfile(
...
CanTxStatus CtrlStepMotor::SetStreamingAngle(float _angle,
                                             const CanTxMetadata* metadata)
{
    _angle = inverseDirection ? -_angle : _angle;
...
CanTxStatus CtrlStepMotor::SetStreamingAngleWithVelocityLimit(
...
CanTxStatus CtrlStepMotor::TryUpdateAngle(const CanTxMetadata* metadata)
{
    constexpr uint8_t mode = 0x23;
    CAN_TxHeaderTypeDef request_header = txHeader;
    request_header.StdId = nodeID << 7 | mode;
