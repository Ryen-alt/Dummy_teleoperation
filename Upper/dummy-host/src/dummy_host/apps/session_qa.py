...
<td>{report.tick_interval_p95_ms:.3f}</td><td>{report.action_samples}</td><td>{report.fault_samples}</td></tr></table>
<h2>Safety action reasons</h2><ul>{reasons}</ul>
<h2>State trajectory</h2>{_trajectory_svg(states)}
<h2>Cameras</h2><table><tr><th>Role</th><th>Unique frames</th><th>Frame gaps</th><th>Latency p95 ms</th><th>RGB/depth skew p95 ms</th><th>Calibration</th></tr>{cameras}</table>
